...
import { Plus, Trash2, FileText, Calendar, Calculator } from 'lucide-react';
import { CreateQuotationData, QuotationLine } from '@/hooks/finance/useCommercialQuotations';
...
export function QuotationForm({ clients, onSubmit, onCancel, isLoading }: QuotationFormProps) {
  const [taxRate] = useState(20);

  const form = useForm<CreateQuotationData>({
    ...
    defaultValues: {
      ...
      valid_until: '',
      notes: '',
      terms_conditions: 'Devis valable 30 jours. TVA applicable selon réglementation en vigueur.',
      lines: [{
        service_name: '',
        description: '',
        quantity: 1,
  ...
  const getValidUntilDefault = () => {
    const date = new Date();
    date.setDate(date.getDate() + 30);
    return date.toISOString().split('T')[0];
  };

  const handleSubmit = async (data: CreateQuotationData) => {
    const processedData = {
      ...data,
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input 
                          type="date" 
                          defaultValue={getValidUntilDefault()}
                          {...field} 
                        />
                      </FormControl>
                      <FormMessage />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea 
                          placeholder="Conditions de vente et modalités..."
                          className="min-h-[100px]"
                          {...field} 
                        />
                      </FormControl>
