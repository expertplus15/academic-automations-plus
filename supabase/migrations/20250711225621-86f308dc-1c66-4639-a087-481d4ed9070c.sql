-- Créer des données de test minimales pour démontrer la détection de conflits

-- D'abord utiliser l'année académique existante ou en créer une simple
INSERT INTO public.academic_years (name, start_date, end_date, is_current, status)
VALUES ('2023-2024 Test', '2023-09-01', '2024-06-30', true, 'active')
ON CONFLICT DO NOTHING;

-- Utiliser une salle existante ou créer une salle simple
WITH room_data AS (
  SELECT id FROM public.rooms LIMIT 1
)
SELECT CASE 
  WHEN EXISTS (SELECT 1 FROM room_data) THEN 'Using existing room'
  ELSE 'No rooms found'
END;

-- Créer des examens de test avec des IDs spécifiques pour les conflits
-- Utiliser des UUIDs simples pour les tests
INSERT INTO public.exams (id, title, exam_type, duration_minutes, status, academic_year_id)
VALUES 
  ('11111111-1111-1111-1111-111111111111'::uuid, 'Test Exam Math', 'written', 120, 'scheduled', 
   (SELECT id FROM public.academic_years WHERE is_current = true LIMIT 1)),
  ('22222222-2222-2222-2222-222222222222'::uuid, 'Test Exam Info', 'written', 90, 'scheduled',
   (SELECT id FROM public.academic_years WHERE is_current = true LIMIT 1))
ON CONFLICT (id) DO UPDATE SET
  title = EXCLUDED.title,
  status = EXCLUDED.status;

-- Créer des sessions d'examens avec CONFLIT INTENTIONNEL dans la même salle
INSERT INTO public.exam_sessions (id, exam_id, room_id, start_time, end_time, status, max_students)
VALUES 
  -- Session 1: Math de 9h à 11h
  ('11111111-1111-1111-1111-111111111111'::uuid, 
   '11111111-1111-1111-1111-111111111111'::uuid,
   (SELECT id FROM public.rooms LIMIT 1),
   '2024-01-20 09:00:00+00', '2024-01-20 11:00:00+00', 'scheduled', 100),
  -- Session 2: Info de 9h30 à 11h dans la MÊME SALLE (CONFLIT!)
  ('22222222-2222-2222-2222-222222222222'::uuid, 
   '22222222-2222-2222-2222-222222222222'::uuid,
   (SELECT id FROM public.rooms LIMIT 1),  -- MÊME SALLE = CONFLIT!
   '2024-01-20 09:30:00+00', '2024-01-20 11:00:00+00', 'scheduled', 80)
ON CONFLICT (id) DO UPDATE SET
  start_time = EXCLUDED.start_time,
  end_time = EXCLUDED.end_time,
  status = EXCLUDED.status;