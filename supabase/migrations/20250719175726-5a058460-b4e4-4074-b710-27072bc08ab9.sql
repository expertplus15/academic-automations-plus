...
-- 6. Mettre à jour les politiques RLS pour inclure l'année académique
-- Students RLS policy mise à jour
DROP POLICY IF EXISTS "Students can view their own data" ON public.students;
DROP POLICY IF EXISTS "Staff can manage students" ON public.students;

CREATE POLICY "Students can view their own data" 
ON public.students FOR SELECT 
USING (profile_id = auth.uid() OR get_current_user_role() = ANY(ARRAY['admin'::user_role, 'hr'::user_role, 'teacher'::user_role]));
