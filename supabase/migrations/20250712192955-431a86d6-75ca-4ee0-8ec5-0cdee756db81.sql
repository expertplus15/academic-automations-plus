-- Correction des données critiques du module académique
...
INSERT INTO public.specializations (id, name, code, description, program_id, level_id, created_at) 
SELECT 
  gen_random_uuid(),
  CASE 
...
-- 2. Créer des données de test pour la table class_groups (classes)
...
-- D'abord créer quelques salles de base si elles n'existent pas
...
-- Maintenant générer des créneaux d'emploi du temps
WITH time_slots AS (
  SELECT 
    day_of_week,
    start_time,
...
-- 4. Corriger les statistiques en créant des fonctions utilitaires
