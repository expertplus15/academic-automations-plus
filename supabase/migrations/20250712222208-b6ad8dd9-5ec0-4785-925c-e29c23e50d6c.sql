-- Correction des données critiques du module académique (version avec group_type valide)
...
-- 2. Créer des données de test pour la table class_groups (classes) avec les bons types
...
SELECT 
  ...
  'normal',  -- Utiliser un type de groupe standard
  CASE 
...
WHERE EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'class_groups' AND table_schema = 'public')
LIMIT 15;

-- 3. Créer quelques salles de base si elles n'existent pas
...
SELECT 
  ...
  CASE 
    ...
    ELSE 'classroom'
  END,
  'Bâtiment ' || CASE WHEN series <= 4 THEN 'A' ELSE 'B' END,
  CASE WHEN series <= 4 THEN 1 ELSE 2 END,
  now()
